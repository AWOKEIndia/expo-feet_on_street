...
// Coordinates that show all of India in a zoomed-out view
const INDIA_VIEW = {
  latitude: 20.5937,
  longitude: 78.9629,
  latitudeDelta: 15,
...
const CheckInConfirmationModal: React.FC<CheckInConfirmationModalProps> = ({
  ...
  const animateToLocation = (newLocation: { latitude: number; longitude: number }) => {
    if (mapRef.current) {
      // First zoom out to show India (if initial load)
      if (initialLoad) {
        mapRef.current.animateToRegion(INDIA_VIEW, 500);

        // Then zoom to user location after a short delay
        setTimeout(() => {
          mapRef.current?.animateToRegion({
            ...newLocation,
            latitudeDelta: 2,  // State-level zoom
            longitudeDelta: 2,
          }, 500);

          // Finally zoom to precise location
          setTimeout(() => {
            mapRef.current?.animateToRegion({
              ...newLocation,
        ...
        // Subsequent updates just zoom to the precise location
        mapRef.current.animateToRegion({
          ...newLocation,
          latitudeDelta: 0.05,
          longitudeDelta: 0.05,
          ...
          <View style={styles.modalContent}>
            ...
            >
              {fetchingLocation
                ...
                : location
                ? "You are about to check in at your current location:"
